...
class Parkir:
    def __init__(self, jenis_kendaraan):
        ...
        self.tarif_per_jam_mobil = 6000
        self.tarif_per_jam_motor = 3000
        self.jam_masuk = None
        self.jam_keluar = None
        self.biaya = None

    def set_jam_masuk(self, jam_masuk):
        self.jam_masuk = datetime.strptime(jam_masuk, '%H:%M')

    def set_jam_keluar(self, jam_keluar):
        self.jam_keluar = datetime.strptime(jam_keluar, '%H:%M')

    def hitung_durasi_parkir(self):
        return self.jam_keluar - self.jam_masuk

    def hitung_biaya_parkir(self):
        durasi_parkir = self.hitung_durasi_parkir().total_seconds() / 3600

        if self.jenis_kendaraan.lower() == "mobil":
            self.biaya = durasi_parkir * self.tarif_per_jam_mobil
        elif self.jenis_kendaraan.lower() == "motor":
            self.biaya = durasi_parkir * self.tarif_per_jam_motor
        else:
            return "Jenis kendaraan tidak valid"

        return self.biaya

    def simpan_data_excel(self):
        try:
            workbook = openpyxl.load_workbook('data_parkir.xlsx')
            sheet = workbook.active
        except FileNotFoundError:
            workbook = openpyxl.Workbook()
            sheet = workbook.active
            sheet.append(["Jenis Kendaraan", "Jam Masuk", "Jam Keluar", "Biaya Parkir"])

        sheet.append([self.jenis_kendaraan, str(self.jam_masuk), str(self.jam_keluar), self.biaya])
        workbook.save('data_parkir.xlsx')


class ParkirGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Aplikasi Sistem Parkir")
        self.root.geometry("400x250")

        self.label_jenis = tk.Label(root, text="Jenis Kendaraan:")
        self.label_jenis.pack()

        self.entry_jenis = tk.Entry(root)
        self.entry_jenis.pack()

        self.label_jam_masuk = tk.Label(root, text="Jam Masuk (HH:MM):")
        self.label_jam_masuk.pack()

        self.entry_jam_masuk = tk.Entry(root)
        self.entry_jam_masuk.pack()

        self.label_jam_keluar = tk.Label(root, text="Jam Keluar (HH:MM):")
        self.label_jam_keluar.pack()

        self.entry_jam_keluar = tk.Entry(root)
        self.entry_jam_keluar.pack()

        self.btn_hitung_biaya = tk.Button(root, text="Hitung Biaya Parkir", command=self.hitung_biaya_parkir)
        self.btn_hitung_biaya.pack()

    def hitung_biaya_parkir(self):
        jenis_kendaraan = self.entry_jenis.get()
        jam_masuk = self.entry_jam_masuk.get()
        jam_keluar = self.entry_jam_keluar.get()

        parkir = Parkir(jenis_kendaraan)
        parkir.set_jam_masuk(jam_masuk)
        parkir.set_jam_keluar(jam_keluar)

        biaya = parkir.hitung_biaya_parkir()
        parkir.simpan_data_excel()

        if isinstance(biaya, str):
            messagebox.showerror("Error", biaya)
        else:
            self.tampilkan_receipt(jenis_kendaraan, jam_masuk, jam_keluar, biaya)

    def tampilkan_receipt(self, jenis_kendaraan, jam_masuk, jam_keluar, biaya):
        receipt_info = f"Jenis Kendaraan: {jenis_kendaraan}\n" \
                       f"Jam Masuk: {jam_masuk}\n" \
                       f"Jam Keluar: {jam_keluar}\n" \
                       f"Biaya Parkir: Rp {int(biaya)}"

        messagebox.showinfo("Receipt Parkir", receipt_info)
